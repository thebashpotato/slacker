 ...
 * should be passed around to other objects like `SlackerWindow` as a shared_ptr,
 * using the `SlackerXConnPtr` alias at the bottom of this file. Since the Display
 * is essentially just a connection to the X server, it needs to be shared, naturally
 * in C++ a reference counted object is the best way to do this as the overhead of
 * counting references (incrementing/decrementing an integer) is negligible at best.
 * */
