...
static const int64_t utfmin[UTF_SIZ + 1] = { 0, 0, 0x80, 0x800, 0x10000 };
static const int64_t utfmax[UTF_SIZ + 1] = { 0x10FFFF, 0x7F, 0x7FF, 0xFFFF,
					     0x10FFFF };

static int64_t utf8decodebyte(const char c, size_t *i)
{
...
///
/// @details The caller has to call free(3) on the returned color scheme when done using it.
SlackerColor *drw_scm_create(Drw *drw, const char *clrnames[], size_t clrcount)
{
	size_t i;
	SlackerColor *ret;

	/* need at least two GLOBAL_COLORSCHEMES for a scheme */
...
int drw_text(Drw *drw, int32_t x, int32_t y, uint32_t w, uint32_t h,
	     uint32_t lpad, const char *text, int32_t invert)
{
	int32_t i, ty, ellipsis_x = 0;
	uint32_t tmpw, ew, ellipsis_w = 0, ellipsis_len;
	XftDraw *d = NULL;
	...
	FcPattern *match;
	XftResult result;
	int32_t charexists = 0, overflow = 0;

	// keep track of a couple codepoints for which we have no match
	enum { nomatches_len = 64 };
	static struct {
		int64_t codepoint[nomatches_len];
		uint32_t idx;
	} nomatches;
	static uint32_t ellipsis_width = 0;
